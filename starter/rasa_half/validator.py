...
def normalise_booking_payload(raw: dict) -> dict:
    ...
    deposit_gdb = parse_currency_gbp(deposit_raw) if deposit_raw else 0
    ...
    party_key = dict(vanue_id=venue_id, date=date, time=time)
    stable_suffix = hashlib.blake2b(json.dumps(party_key, sort_keys=True).encode(), digest_size=8).hexdigest()
